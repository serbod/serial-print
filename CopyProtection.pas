...
type
  ...
  function GetDiskCSerial(): string;
  function CheckProtectionKey(sKey: string): Boolean;
...
begin
  ...
  if GetVolumeInformation('c:\', Buffer, SizeOf(Buffer),
    @SerialNum, a, b, nil, 0) then Result:= IntToStr(SerialNum);
end;
...
begin
  sDict:='8462795130';
  l:=12;
  sSerialC:=GetDiskCSerial();
  i:=1;
  while Length(sSerialC)<l do
  begin
    sSerialC:=sDict[i]+sSerialC;
    Inc(i);
    if i>Length(sDict) then i:=1;
  end;
  Result:=Copy(md5(sSerialC), 1, l);
end;

function CheckProtectionKey(sKey: string): Boolean;
begin
  Result:=(StripSpaces(sKey) = GetProtectionKey());
end;

function StripSpaces(Str: string): string;
begin
  Result:=LowerCase(Copy(Str, 1, maxint));
