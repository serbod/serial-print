...
{------------------------------------------------------------------------------}
{  Use GDI+ for smooth preview and thumbnail drawing, and TIF output           }
{  Get the library at http://www.progdigy.com                                  }
{------------------------------------------------------------------------------}
{.$DEFINE GDI_PLUS}

{------------------------------------------------------------------------------}
{  If you need transparent image printing, define IMAGE_TRANSPARENCY or at     }
{  at runtime set *AllowTransparentDIB* global variable to True.               }
{                                                                              }
...
uses
  ...
  {$IFDEF GDI_PLUS}, GDIPAPI, GDIPOBJ, GDIPUTIL {$ENDIF};
...
type
  ...
  protected
    ...
    constructor CreateInStorage(AOwner: TMetafileList;
      const AOffset: TOffset; ASize: DWORD);
  ...
  public
    ...
    {$IFDEF GDI_PLUS}
    property GPMetafile: TGPMetafile read FGPMetafile;
    {$ENDIF}
  end;
  ...
  private
    ...
    FOnPrintAnnotation: TPreviewPageDraw;
    FOnPrintBackground: TPreviewPageDraw;
  ...
  published
    ...
    property OnPrintAnnotation: TPreviewPageDraw read FOnPrintAnnotation write FOnPrintAnnotation;
    property OnPrintBackground: TPreviewPageDraw read FOnPrintBackground write FOnPrintBackground;
  end;
...
var
  AllowTransparentDIB: Boolean = {$IFDEF IMAGE_TRANSPARENCY} True {$ELSE} False {$ENDIF};
...
{.$DEFINE CHECK_CACHE} // Just to check how my poor caching works
...
type
  TdsPDF = record
    Handle: HMODULE;
    BeginDoc: function(FileName: PAnsiChar): Integer; stdcall;
    ...
    PrintPageFile: function(FileName: PAnsiChar): Integer; stdcall;
...
const
  ...
  SNotEnoughMemory     = 'There is not enough memory to create a new page';
...
{$IFDEF CHECK_CACHE}
var
  CacheRequestCount: Integer = 0;
  CacheReplacedCount: Integer = 0;
  CacheReadCount: Integer = 0;
  CacheWriteCount: Integer = 0;
{$ENDIF}
...
{$IFDEF GDI_PLUS}
procedure SmoothDraw(Canvas: TCanvas; const Rect: TRect; Image: TGPImage);
var
...
begin
  ...
  try
    ...
    with Rect do Graphics.DrawImage(Image, Left, Top, Right - Left, Bottom - Top);
  finally
    Graphics.Free;
  end;
end;
...
begin
  ...
  {$IFDEF GDI_PLUS}
  FGPMetafile := TGPMetafile.Create(FMetafile.Handle, false);
  {$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      FOffset := FOwner.Storage.Seek(FOffset, soFromEnd);
      FMetafile.SaveToStream(FOwner.Storage);
      FSize := FOwner.Storage.Position - FOffset;
    end;
...
begin
  {$IFDEF CHECK_CACHE}
  if CacheRequestCount <> 0 then
  begin
    OutputDebugString(PChar(Format('[ Number Of Entries: %d ]', [FEntries.Count])));
    OutputDebugString(PChar(Format('[ Cache Size: %d ]', [FCacheSize])));
    OutputDebugString(PChar(Format('[ Cache Requests: %d ]', [CacheRequestCount])));
    OutputDebugString(PChar(Format('[ Cache Replacement Rate: %%%.1f ]', [100 * CacheReplacedCount / CacheRequestCount])));
    OutputDebugString(PChar(Format('[ Disk Reads: %d ]', [CacheReadCount])));
    OutputDebugString(PChar(Format('[ Disk Writes: %d ]', [CacheWriteCount])));
  end;
  {$ENDIF}
...
begin
  ...
  if Assigned(FOnMultipleChange) then
    FOnMultipleChange(Self, Index, Count - 1);
end;
...
begin
  ...
  begin
    ...
    begin
      if Index1 < Index2 then
        FOnMultipleChange(Self, Index1, Index2)
      else
        FOnMultipleChange(Self, Index2, Index1);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if Index < NewIndex then
        FOnMultipleChange(Self, Index, NewIndex)
      else
        FOnMultipleChange(Self, NewIndex, Index);
    end;
  end;
end;
...
begin
  ...
  if MetafilesSignature <> Signature then
  ...
  try
    ...
    begin
      FStorage := TTemporaryFileStream.Create;
      FStorage.CopyFrom(Stream, Stream.Size - Stream.Position);
      Offset := 0;
      for I := 0 to Offsets.Count - 1 do
      begin
  ...
  if (FEntries.Count > 0) and Assigned(FOnMultipleChange) then
    FOnMultipleChange(Self, 0, Count - 1);
end;
...
var
  ...
  Offsets: TIntegerList;
  Entry: TMetafileEntry;
  HeaderOffset, BaseOffset: TOffset;
  I: Integer;
begin
  Signature := MetafilesSignature;
...
begin
  ...
  begin
    ...
    TPaperPreview(Source).Hint := Hint;
  end
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      Flags := TextAlignFlags[Alignment] or TextWordWrapFlags[WordWrap] or DT_NOPREFIX;
      {$IFDEF COMPILER4_UP}
  ...
  begin
    ...
    Rect.Top := Height - CaptionHeight - ShadowSize;
  ...
  begin
    ...
    Canvas.Pen.Color := BorderColor;
    Canvas.Brush.Style := bsClear;
    Canvas.Rectangle(0, 0, Width - ShadowSize, Height - CaptionHeight - ShadowSize);
  end;
end;
...
begin
  if ShowCaption then
  begin
    Flags := TextAlignFlags[Alignment] or TextWordWrapFlags[WordWrap] or DT_NOPREFIX or DT_CALCRECT;
    {$IFDEF COMPILER4_UP}
    Flags := DrawTextBiDiModeFlags(Flags);
    {$ENDIF}
    Rect.Left := 0;
    Rect.Right := Width - ShadowSize;
    Rect.Top := 0;
    Rect.Bottom := 0;
    Dec(Rect.Right, ShadowSize);
...
begin
  FPageRect.Left := BorderSize;
  FPageRect.Top := BorderSize;
...
begin
  Result := Value + 2 * FBorderSize + FShadowSize + CaptionHeight;
end;
  ...
  case InUnits of
    ...
    mmPixel:
      case OutUnits of
        ...
        mmPixel: Result := Round(Value * DPI / Screen.PixelsPerInch);
      end;
  end;
end;
    ...
    begin
      ...
      begin
        case LoWord(Message.WParam) of
          MK_CONTROL: Zoom := Zoom + ZoomStep;
          MK_SHIFT, MK_MBUTTON: CurrentPage := CurrentPage - 1;
          0: Perform(WM_VSCROLL, SB_LINEDOWN, 0);
        else
          Message.Result := 0;
        end;
      end;
...
begin
  FAborted := FAborted or not Succeeded;
...
begin
  ...
  OffsetRect(Result, -FPaperView.BorderSize, -FPaperView.BorderSize);
...
begin
  ...
  begin
    case FZoomState of
      zsZoomOther:
      begin
        Result.X := ActualWidth(MulDiv(FDeviceExt.X, FZoom, 100));
        Result.Y := ActualHeight(MulDiv(FDeviceExt.Y, FZoom, 100));
      end;
      zsZoomToWidth:
      begin
        Result.X := Space.X;
        Result.Y := ActualHeight(MulDiv(LogicalWidth(Result.X), FDeviceExt.Y, FDeviceExt.X));
      end;
      zsZoomToHeight:
      begin
        Result.Y := Space.Y;
        Result.X := ActualWidth(MulDiv(LogicalHeight(Result.Y), FDeviceExt.X, FDeviceExt.Y));
      end;
      zsZoomToFit:
      begin
    ...
    if FZoomState <> zsZoomOther then
      FZoom := Round((100 * LogicalHeight(Result.Y)) / FDeviceExt.Y);
  end;
end;
...
begin
  if (csLoading in ComponentState) or
     ((FPageList.Count = 0) and not (csDesigning in ComponentState))
  then
    Exit;
  ...
  try
    ...
    begin
      ...
      begin
        if X >= 0 then
          HorzScrollbar.Position := MulDiv(X, HorzScrollBar.Range, FDeviceExt.X)
      ...
      begin
        if Y >= 0 then
          VertScrollBar.Position := MulDiv(Y, VertScrollBar.Range, FDeviceExt.Y)
        else if FZoomSavePos then
  ...
  begin
    Percent.X := (MulDiv(100, FPaperView.Width, FDeviceExt.X) div FZoomStep) * FZoomStep;
    Percent.Y := (MulDiv(100, FPaperView.Height, FDeviceExt.Y) div FZoomStep) * FZoomStep;
...
begin
  ...
  begin
    {$IFDEF GDI_PLUS}
    SmoothDraw(Canvas, Rect, FPageList.Items[PageNo-1].GPMetafile);
    {$ELSE}
    Canvas.StretchDraw(Rect, FPageList[PageNo-1]);
    {$ENDIF}
  end
  ...
  begin
    ...
    try
      ...
      {$IFDEF GDI_PLUS}
      SmoothDraw(Bitmap.Canvas, Rect, FPageList.Items[PageNo-1].GPMetafile);
      {$ELSE}
      Bitmap.Canvas.StretchDraw(BitmapRect, FPageList[PageNo-1]);
      {$ENDIF}
...
begin
  ...
  try
    ...
    FPageList.LoadFromStream(DecoderStream);
    {$IFDEF ZLIB}
...
begin
  ...
  try
    ...
    EncoderStream.Write(Data, SizeOf(Data));
    FPageList.SaveToStream(EncoderStream);
    {$IFDEF ZLIB}
    EncoderStream.Free;
    {$ENDIF}
  finally
...
{$IFDEF GDI_PLUS}
  ...
  function PageImageOf(PageNo: Integer): TGPImage;
  var
    Metafile: TMetafile;
    Bitmap: TBitmap;
  begin
    Metafile := Pages[PageNo];
    Bitmap := TBitmap.Create;
    try
      Bitmap.Canvas.Brush.Color := clWhite;
...
begin
  ...
  try
    ...
    try
      ...
      MultiPage.Save(FileName, EncoderClsid, @EncoderParameters);
      for PageNo := 2 to TotalPages do
      begin
        Page := PageImageOf(PageNo);
        try
          ParameterValue := EncoderValueFrameDimensionPage;
          MultiPage.SaveAdd(Page, @EncoderParameters);
        finally
          Page.Free;
        end;
      end;
...
begin
  ...
  begin
    ...
    try
      dsPDF.BeginDoc(PAnsiChar(AnsiString(FileName)));
      try
        Mem := TMemoryStream.Create;
        try
          for PaegNo := 1 to TotalPages do
          begin
            Mem.Clear;
            Pages[PaegNo].SaveToStream(Mem);
            if PaegNo > 1 then dsPDF.NewPage;
...
begin
  if FirstPage < 1 then
    FirstPage := 1;
  if LastPage > TotalPages then
    LastPage := TotalPages;
  if (FState = psReady) and PrinterInstalled and (FirstPage <= LastPage) then
  begin
    Done := False;
    FAborted := False;
    ...
    try try
      ...
      begin
        DoProgress(PageNo, PrintedCount, PageCount);
        if FAborted then Exit;
        if PageNo > FirstPage then Printer.NewPage;
        PrintPage(PageNo, Printer.Canvas, PageRect);
        Inc(PrintedCount);
        DoProgress(PageNo, PrintedCount, PageCount);
        if FAborted then Exit;
      end;
...
begin
  ...
  begin
    ...
    try try
      ...
      begin
        DoProgress(Pages[I], PrintedCount, Pages.Count);
        if FAborted then Exit;
...
begin
  ...
  begin
    SendMessage(WindowHandle, WM_SETREDRAW, 0, 0);
    PageWidth := MulDiv(PrintPreview.PagePixels.X, Zoom, 100);
    PageHeight := MulDiv(PrintPreview.PagePixels.Y, Zoom, 100);
