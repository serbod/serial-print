...
uses
  ...
  {$IFDEF COMPILER4_UP} ImgList, {$ENDIF} Preview;
...
type
  ...
  private
    ...
    procedure DrawRandomShapesPage;
...
uses
  {$IFDEF COMPILER7_UP} XPMan, {$ENDIF} Math;
  ...
  begin
    ...
    case PrintPreview.State of
      ...
      begin
        ...
        StatusBar.Panels[5].Text := 'Creating pages...';
      end;
      psLoading:
        StatusBar.Panels[5].Text := 'Loading pages from file...';
      psSaving:
        StatusBar.Panels[5].Text := 'Saving pages to file...';
      psSavingPDF:
        StatusBar.Panels[5].Text := 'Saving pages as PDF...';
      psSavingTIF:
        StatusBar.Panels[5].Text := 'Saving pages as multiframe TIF...';
      psPrinting:
        StatusBar.Panels[5].Text := 'Preparing to print pages...';
    end;
  end;
  Update;
end;
...
procedure TMainForm.PrintPreviewPrintProgress(Sender: TObject; PageNum,
  Progress: Integer; var AbortIt: Boolean);
begin
  StatusBar.Panels[5].Text := Format('Printing page %d of %d (%%%d done)...',
    [PageNum, PrintPreview.TotalPages, Progress]);
  Update;
end;
...
begin
  ...
  begin
    Canvas.Pen.Width := XFrom(mmLoMetric, 5); { 0.05mm }
...
begin
  ...
  begin
    ...
    // In addition, we want the image height does not exceed 3cm limit.
    ...
    Canvas.Font.Size := 12;
    // while we have not reached to the frame's bottom (2mm space), do...
...
begin
  ...
  begin
    ...
    with PointFrom(mmLoMetric, 50, 50) do { 0.5cm additional margin }
      InflateRect(PR, -X, -Y);
...
begin
  ...
  begin
    PR := PageBoundsAfterMargin;
    with PointFrom(mmLoMetric, 100, 100) do { 1cm additional margin }
      InflateRect(PR, -X, -Y);
    PaintRichText(PR, RichEdit1, 0, nil);
  end;
end;

procedure TMainForm.DrawRandomShapesPage;
var
  PR, R: TRect;
  I: Integer;
begin
  ...
  begin
    ...
    Canvas.Pen.Width := XFrom(mmLoMetric, 1); { 0.1mm }
    for I := 1 to 10 do
    begin
      Canvas.Pen.Color := RGB(Random(256), Random(256), Random(256));
      Canvas.Brush.Color := RGB(Random(256), Random(256), Random(256));
      R.Left := PR.Left + Random(PR.Right - PR.Left);
      R.Top := PR.Top + Random(PR.Bottom - PR.Top);
      R.Right := R.Left + Random(PR.Right - R.Left);
      R.Bottom := R.Top + Random(PR.Bottom - R.Top);
      Canvas.RoundRect(R.Left, R.Top, R.Right, R.Bottom,
        Random(R.Right - R.Left), Random(R.Bottom - R.Top));
    end;
  end;
end;
